...
function SideDishes() {
    ...
    useEffect(()=>{
      ...
      const isSideDishes = foodItems.filter(item=> item.category === 'others')
  return (  
        <div>
            <div className='text-white text-lg md:text-xl xl:text-2xl mb-2 font-semibold'>SideDishess</div>
            <div className='max-w-[1200px] xl:w-full xl:grid-cols-2 lg:w-[700px] md:gap-5 grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 md:max-w-[880px] xl:max-w-[880px] md:grid-cols-2 lg:max-w-[500px] lg:grid-cols-1 gap-6'>
                {
                isSideDishes && isSideDishes.map((item, idx) => {
                    return (
